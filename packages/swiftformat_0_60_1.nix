...
let
  src =
    if stdenv.isDarwin then
      sources.swiftformat-0_60_1-darwin.src
    else if stdenv.isLinux && stdenv.hostPlatform.isAarch64 then
      sources.swiftformat-0_60_1-linux-aarch64.src
    else if stdenv.isLinux && stdenv.hostPlatform.isx86_64 then
      sources.swiftformat-0_60_1-linux-x86_64.src
    else
      throw "Unsupported platform: ${stdenv.hostPlatform.system}";

  version = sources.swiftformat-0_60_1-darwin.version;
in
stdenv.mkDerivation {
  pname = "swiftformat";
  inherit version src;
